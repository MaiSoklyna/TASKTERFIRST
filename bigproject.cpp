...
public:
    ...
    {
        file >> IDPeople;
        file.ignore();             // Skip space after IDPeople
        getline(file, nameP, ' '); // Read until space
        file >> age >> day >> mm >> yy;
        file.ignore();                   // Skip space after date
        getline(file, numberPhone, ' '); // Read until space
        file >> IDCake;
        file.ignore();                 // Skip space after IDCake
        getline(file, colorcake, ' '); // Read until space
        getline(file, timeOrder, ' '); // Read until space
        getline(file, timeGet, ' ');   // Read until space
        getline(file, typeCake, ' ');  // Read until space
        file >> sizeCake >> qualityCake >> price >> total;
        file.ignore(); // Skip end of line
    }
...
void loadData(Shop shop[], int &listcount, int &cakecount)
{
    ...
    while (file.peek() != EOF) // Avoid reading past the end of the file
    {
        Shop s;
        s.loadFromFile(file);
        shop[listcount] = s;
